...
namespace RTEditor
{
            ...
            if (_deletedObjects != null)
            {
                ...
                foreach (var deletedObject in _deletedObjects)
                {
                    if (deletedObject == null)
                        _deletedObjects.Remove(deletedObject);
                    else
    ...
    public class CreateObjectAction : IUndoableAndRedoableAction, IAction
    {
        /// <summary>
        /// After the action is executed, this will hold all objects which were deeted.
        /// </summary>
        ...
        public CreateObjectAction(GameObject sourceObject) {
            if (sourceObject != null)
            {
                _createdObject = sourceObject;
            }
        }
        ...
        public void Execute()
        {
                ...
                // Inform the selection module that the objects were deleted
                EditorObjectSelection.Instance.OnCreated(_createdObject);
        ...
        {
            // When the action is popped of the stack, we need to make sure that if the
            // objects are in their create state (they might not be if the action was
            // undone), we actually create them from the scene. Otherwise, they would 
            // just be left behind.
            //Debug.Log("移除队列"+_createdObject);
            //if (_createdObject != null )
            //{
            //    // We can check if the objects are in the deleted state, if the first
            //    // (or any) element inside the array is inactive.
            //    if (!_createdObject.activeInHierarchy)
            //    {
            //        // Delete the objects
